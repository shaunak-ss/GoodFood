          ...
          <tbody>
            {data.map((food, index) => (
              <tr>
                ...
                <td>{food.size}</td>
                <td>{food.price}</td>
                <td ><button type="button" className="btn p-0 bg-black"><Delete onClick={() => { dispatch({ type: "REMOVE", index: index }) }} /></button> </td></tr>
            ))}
          </tbody>
        </table>
        <div><h1 className='fs-2 text-black'>Total Price: {totalPrice}/-</h1></div>
        <div>
          <button className='btn bg-warning mt-5 text-black' onClick={handleCheckOut} > Check Out </button>
        </div>
      </div>
